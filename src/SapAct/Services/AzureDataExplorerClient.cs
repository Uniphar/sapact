...
/// <param name="cslAdminProvider">
///     The <see cref="ICslAdminProvider" /> instance that allows to execute Azure Data Explorer
///     control commands.
/// </param>
/// <param name="kustoIngestClient">
///     The <see cref="IKustoIngestClient" /> instance that allows to ingest data into Azure
///     Data Explorer.
/// </param>
...
) : IAzureDataExplorerClient
{
    ...
    public async Task<IEnumerable<(string name, string type)>> GetCurrentColumnListAsync(string tableName, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var result = await cslQueryProvider.ExecuteQueryAsync(cslAdminProvider.DefaultDatabaseName, getSchemaCommand, new(), cancellationToken);
    ...
    /// </summary>
    /// <param name="targetSchema">The target schema for the table.</param>
    /// <param name="cancellationToken">
    ///     The <see cref="CancellationToken" /> instance to signal the request to cancel the
    ///     operation.
    /// </param>
    /// <param name="tableName">The name of the table to create or update.</param>
    ...
    public async Task CreateOrUpdateTableAsync(string tableName, List<ColumnDefinition> targetSchema, CancellationToken cancellationToken = default)
    {
        ...
        var createTableMappingCommand = CslCommandGenerator.GenerateTableMappingCreateOrAlterCommand(IngestionMappingKind.Json, tableName, $"{tableName}JSONMapping", mapping);
    ...
    /// <param name="ingestClient">
    ///     The <see cref="IKustoIngestClient" /> instance that allows to ingest data into Azure Data
    ///     Explorer.
    /// </param>
    ...
    private async Task IngestDataWithClientAsync<TData>(IKustoIngestClient ingestClient, TData data, string tableName, string tableMapping)
    {
        ...
        await ingestClient.IngestFromStreamAsync(dataStream,
            new(cslAdminProvider.DefaultDatabaseName, tableName)
            {
