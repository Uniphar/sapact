...
public static class JsonElementExtensions
{
    ...
    public static List<ColumnDefinition> GenerateColumnList(this JsonElement payload, TargetStorageEnum targetStorage)
    {
        ...
        if (targetStorage is TargetStorageEnum.LogAnalytics)
        {
            return propDefinitions
                .Append(new() { Name = "TimeGenerated", Type = "datetime" })
                .ToList();
        }
    ...
    public static Dictionary<string, string> ExportToFlattenedDictionary(this JsonElement payload)
    {
        ...
        foreach (var field in payload.EnumerateObject().Where(x => x.Name != DataColumnName)) dataFields[field.Name] = field.Value.ToString();

        return dataFields;
    }

    public static BinaryData GenerateBinaryData(this JsonElement jsonElement) => BinaryData.FromObjectAsJson(new[] { jsonElement.ExportToFlattenedDictionary() });

    public static string ExportDCRImmutableId(this JsonElement jsonElement) => jsonElement.GetProperty("properties").GetProperty("immutableId").GetString()!;
